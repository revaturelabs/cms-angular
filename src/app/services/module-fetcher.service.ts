...
export class ModuleFetcherService {
    ...
    updateModuleLinks(module: Module): Observable<HttpHeaderResponse> {

        let body: string = JSON.stringify(module.links);
        globalCacheBusterNotifier.next();

        return this.http.put<HttpHeaderResponse>(
            this.endpoints.UPDATE_MODULE_LINKS.replace("${id}", module.id.toString()), body, { headers: this.HEADERS });
    }
